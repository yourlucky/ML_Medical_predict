...
if __name__ == '__main__':
    ...
    #기본적 데이터로드
    _data = pd.read_csv('/Users/yoon/Documents/lecture_note/760/medical_project/yoon_python/data_bioage.csv')
    ...
    print("{} : {:.2f}%".format(model, count/len(Y_pred)*100))

    #2단계 클러스터 2개 그룹 해보기

    # 클러스터 2개 만들고 붙이는 부분
    # g_data = pd.read_csv('/Users/yoon/Documents/lecture_note/760/medical_project/yoon_python/data_bioage.csv')
    # g_data = g_data[g_data['_death_in_3yr']==0]
    ...
    # Model = KNeighborsClassifier(n_neighbors=10) #75.49
    # # #Model = GaussianNB() #<-뭘까????/ㅜㅜ 1%

    # # # Model =MLPClassifier(solver='sgd', hidden_layer_sizes=(20,20,5),  #76.49%(7개) 93%(5개)
    # # #                        learning_rate_init=0.001, max_iter=10000, activation='relu',power_t=0.2)
    ...
    #g_data.to_csv('/Users/yoon/Documents/lecture_note/760/medical_project/yoon_python/data_group.csv')
    g_data = pd.read_csv('/Users/yoon/Documents/lecture_note/760/medical_project/yoon_python/data_group.csv')
    ...
    for i in range(0,len(Y_pred_one)) :
        if abs(Y_pred_one[i]-y_test[i])<=3 or abs(Y_pred_two[i]-y_test[i]) <=3 :
            count+=1

    #print("{} : {:.2f}%".format(model_one, count/len(Y_pred_one)*100))
