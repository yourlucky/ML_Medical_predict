...
def setBiologicalAge(data, col, stat_table):
    ...
    for row in range(0, len(data)):
        if _data.at[row, 'Sex'] == 0: # female
            _data.at[row, col] = stat_table.at[_data.at[row, 'Age at CT'], 'Female'] - _data.at[row, d_col]/365 + _data.at[row, 'Age at CT']
        else: # male
            _data.at[row, col] = stat_table.at[_data.at[row, 'Age at CT'], 'Male'] - _data.at[row, d_col]/365 + _data.at[row, 'Age at CT']


    bio_mean = _data[col].mean()
    age_mean = _data['Age at CT'].mean()
    bio_max = _data[col].max()
    bio_min = _data[col].min()
