...
if __name__ == '__main__':
    ...
    _data = pd.read_csv('data_group.csv')
    ...
    print("1step {} : {:.2f}%".format(model, count/len(Y_pred)*100))
    ...
    # #평균 testset 만들기
    _test=Bio_answer(l_data)
   
    test_x= _test[columns]
    y_test=_test['I_age']

    #예측한 그룹이 1 인것과 2인것을 각각 생성
    G_data_one = l_data[l_data['Group']==1]
    G_data_two = l_data[l_data['Group']==2]
